...
image = modal.Image.debian_slim(python_version="3.12").pip_install("python-fasthtml==0.10.0")
...
def serve():
    ...
    @app.get("/")
    def home():
            input_remaining_km = fh.Input(id="remaining_km", name="remaining_km", type=int, placeholder="Enter remaining km", hx_validate="false")
            input_remaining_percentage = fh.Input(id="remaining_percentage", name="remaining_percentage", type=int, placeholder="Enter remaining battery in %", hx_validate="false")
            input_max_range_km = fh.Input(id="max_range_km", name="max_range_km", type=int, placeholder="Enter max. range (km)")
            input_max_battery_capacity_kwh = fh.Input(id="max_battery_capacity", name="max_battery_capacity_kwh", type=float, placeholder="Enter max. battery (kWh)")
            input_max_charging_rate_kwh = fh.Input(id="max_charging_rate_kwh", name="max_charging_rate_kwh", type=float, placeholder="Enter max. charging rate (kWh)")
            input_finished_at = fh.Input(id="finish_at_str", name="finish_at_str", type=str, placeholder="Enter finish time (6:00)")
            ev_form = fh.Form(method="post", action='/calculate')(
                ...
                fh.Fieldset(fh.Label('Battery Capacity (kWh)', input_max_battery_capacity_kwh)),
            ...
            fh.H1(TITLE),
            fh.H2(DESCRIPTION), ev_form,  cls="container"
        )
